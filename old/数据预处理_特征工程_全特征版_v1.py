...
def add_prophet_dynamic_features_sliding(df):
    print("正在生成Prophet动态特征（滑窗无泄漏）...")
    result_list = []
...
def main():
    ...
    train_df = add_prophet_static_features(train_df)
    test_df = add_time_features(test_df)
    test_df = add_discount_price_features(test_df)
    test_df = add_lag_features(test_df)
    test_df = add_promotion_features(test_df)
    test_df = add_prophet_static_features(test_df)
    # Prophet动态特征（滑窗无泄漏）
    train_df = add_prophet_dynamic_features_sliding(train_df)
    test_df = add_prophet_dynamic_features_sliding(test_df)
    # 归一化前统一处理inf和nan
    ...
    # 保存处理后的数据
    train_file = '预处理后的商品数据_训练集_全特征_v1.csv'
    test_file = '预处理后的商品数据_测试集_全特征_v1.csv'
