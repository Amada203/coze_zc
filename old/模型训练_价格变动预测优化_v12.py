...
# 3. 生成Prophet衍生特征
def add_prophet_derived_features(df, add_residual=True):
    df['yhat_diff'] = df.groupby('sku_id')['yhat'].diff()
    if add_residual:
        df['yhat_residual'] = df['discount_price'] - df['yhat']
add_prophet_derived_features(train_df, add_residual=True)
add_prophet_derived_features(test_df, add_residual=False)  # 测试集不生成yhat_residual，防止泄漏
...
feature_cols_train = [
    ...
    'trend', 'yearly', 'weekly',
    'yhat', 'yhat_diff', 'yhat_residual'  # 训练集可用残差
]
feature_cols_test = [
    'discount_price_lag_1', 'discount_price_lag_3', 'discount_price_lag_7',
    'discount_price_mean_7', 'discount_price_std_7', 'discount_price_min_7',
    'discount_price_max_7', 'discount_price_trend_7',
    'trend', 'yearly', 'weekly',
    'yhat', 'yhat_diff'  # 测试集不能用残差
]

# 7. 去除因滞后/预测导致的NaN
train_df = train_df.dropna(subset=feature_cols_train + ['label'])
test_df = test_df.dropna(subset=feature_cols_test + ['label'])
...
# 9. 特征与标签相关性分析
print("\n【特征与标签相关性分析】")
correlations = train_df[feature_cols_train + ['label']].corr()['label'].sort_values(ascending=False)
print(correlations)
try:
...
# 测试集只保留与训练集交集的特征
important_features_test = [f for f in important_features if f in X_test.columns]
X_test = X_test[important_features_test]
...
# 保存评估结果
with open('模型评估报告_v12.txt', 'w', encoding='utf-8') as f:
    f.write("K折交叉验证评估结果:\n")
...
results_df.to_csv('discount_price变动预测结果_v12.csv', index=False)
...
print("\n详细评估结果已保存到'模型评估报告_v12.txt'", flush=True)
print("预测结果已保存到'discount_price变动预测结果_v12.csv'", flush=True) 