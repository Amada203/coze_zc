...
# 保存评估结果
with open('模型评估报告_全特征版.txt', 'w', encoding='utf-8') as f:
    f.write("K折交叉验证评估结果:\n")
...
results_df.to_csv('discount_price变动预测结果_全特征版.csv', index=False)
...
print("\n详细评估结果已保存到'模型评估报告_全特征版.txt'", flush=True)
print("预测结果已保存到'discount_price变动预测结果_全特征版.csv'", flush=True) 