...
# 保存评估结果
with open('模型评估报告_v7.txt', 'w', encoding='utf-8') as f:
    f.write("K折交叉验证评估结果:\n")
    ...
    feature_importance = pd.DataFrame({
        ...
        'importance': best_model.get_score(importance_type='weight').values()
    })
    feature_importance = feature_importance.sort_values('importance', ascending=False)
    for feature, importance in best_model.get_score(importance_type='weight').items():
...
results_df.to_csv('商品价格变动预测结果_v7.csv', index=False)
...
print("\n详细评估结果已保存到'模型评估报告_v7.txt'")
print("预测结果已保存到'商品价格变动预测结果_v7.csv'") 