...
# 保存评估结果
with open('模型评估报告_全特征版_lightgbm_v1.txt', 'w', encoding='utf-8') as f:
    f.write("K折交叉验证评估结果:\n")
    ...
    f.write(f"LightGBM模型精确率: {precision_test:.4f}\n")
    f.write(f"LightGBM模型召回率(重点): {recall_test:.4f}\n")
    f.write(f"LightGBM模型F1分数: {f1_test:.4f}\n")
    f.write(f"LightGBM模型PR-AUC: {pr_auc_test:.4f}\n")
    f.write(f"LightGBM模型Logloss: {logloss_test:.4f}\n")
    f.write(f"LightGBM模型混淆矩阵: {cm_test.tolist()}\n")
    f.write("\n特征重要性:\n")
...
results_df.to_csv('discount_price变动预测结果_全特征版_lightgbm_v1.csv', index=False)
...
print(f"LightGBM模型精确率: {precision_test:.4f}", flush=True)
print(f"LightGBM模型召回率(重点): {recall_test:.4f}", flush=True)
print(f"LightGBM模型F1分数: {f1_test:.4f}", flush=True)
print(f"LightGBM模型PR-AUC: {pr_auc_test:.4f}", flush=True)
print(f"LightGBM模型Logloss: {logloss_test:.4f}", flush=True)
print(f"LightGBM模型混淆矩阵: {cm_test.tolist()}", flush=True)
print("\n详细评估结果已保存到'模型评估报告_全特征版_lightgbm_v1.txt'", flush=True)
print("预测结果已保存到'discount_price变动预测结果_全特征版_lightgbm_v1.csv'", flush=True)

# 保存最佳模型
joblib.dump(best_model, 'lightgbm_price_change_model_v1.pkl')
print("模型已保存为 lightgbm_price_change_model_v1.pkl")
